...
object like a respresentation of a TV programme that contains a title, synopsis
and preview image.
...
Ad now point here doe have any code, that comes at the next tiers where you
start writing these common ideas into your application.
...
Our two projects: the 2013 redesign - so named retroactivly because we just
...
producing high fidelity page designs which are the implememnted by a team of
...
respect that difference and thats what ultimatly lead to my disatisfaction with
the Amen project.
...
backwards incompatable changes and how to deal with variations of a component
...
what we could acomplish given amount of time and manpower afforded to us.
...
This difference in our vision and our ability to execute was particulary stark
...
swiveling your chair and asking the person next to you to provide context on why
something was done in a particular way. Our styleguide still existed and
...
devs overall meant that oral tradition wouldn't really work - interuping a
member of your own team is fine, but you're a probably little more reticent to
ask an someone external. There was also now twice as many people to onboard onto
...
didn't want developers familar with the system explaining the same concepts
...
Once again I dropped the ball. The /radio team were unfamilar with the process
...
This failure to adequatly provide mentoring in the early stages meant that in
...
just about the hows of the system but you also need to set expections around its
usage - often, these components are suggestions for common cases and it is fine
to diverge from them and you need to clearly communicate that ideal.
...
maintenace was straightforward. Building for a single app we could build the
...
complicated ongoing maintenace compared to the 2013 system. We now had to deal
with releasing, version numbers, managing breaking changes. Having a change to a
component be visible within the app also be came more involved as you had to
make your changes in Amen, get it merged, cut a release, then update your app to
the newest version of Amen - three extra steps compared to modifying components
that directly lived within the app. We eventually got into the swing of
releasing of new versions though. We ended up having an irregular release
schedule based around when a team needed new features in Amen. An app team was
free to make changes and cut a new release at almost any time as we kept the
master branch clean and all work in progress occured within feature branches.
...
a long time which meant the system grew to be more complex that necessary.
...
If you have a decated component library and styleguide you need ensure you have
the time to work on paying down technical debt and to invest in feautres that
improve the developer experience of using the library and styleguide.
...
Underestimating the additional effort required when creating a shared system
that was our downfall. Amen overreached - we tried to do something with
increased scope but we didn't have the dedicated time to invest in the
cross-team education and stewardship of the shared components to ensure it was a
success. Our vision was greater than our ability to execute.

Amen had the goal of reducing effort through sharing code, however all that
sharing acomplished was tying together two distinct products that had slightly
...
consumers educate them on how to use and contribuite back to the system and to
...
of using it and that allows it to be sucessful.
